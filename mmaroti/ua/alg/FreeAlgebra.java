...
public class FreeAlgebra implements Algebra
{
	...
	public static boolean isRedundantFactor(List<Algebra> algebras, List<List<Integer>> generators,
		int target, int testSize)
	{
	...
	public int size() { return algebra.size(); }
	public Function[] operations() { return algebra.operations(); }
		...
		while( iter.hasNext() )
		{
			...
			do
			{
				...
				if( isRedundantFactor(algebras, generators, target, 1) ||
					isRedundantFactor(algebras, generators, target, 2) )
				{
					removeFactor(algebras, generators, target);
				}
			} while( arg.next() );
