...
public class FactorAlgebra implements Algebra
{
	...
	public int size() { return concreteElems.length; }
	
	public int toAbstractElem(int elem) { return abstractElems[elem]; }
	public int toConcreteElem(int block) { return concreteElems[block]; }
	...
	public Function[] relations() { return new Function[0]; }
	...
	public FactorAlgebra(Algebra algebra, Equivalence equ)
	{
		...
		{
			abstractElems[i] = (repr[i] != i) ? 
				abstractElems[repr[i]] : blockCount++;
		}
	...
	public static Equivalence congruence(Algebra algebra, LinkedList<IntPair> pairs)
	{
		int size = algebra.size();
		Function[] operations = algebra.operations();
