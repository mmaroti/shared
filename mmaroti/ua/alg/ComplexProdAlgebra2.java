...
public class ComplexProdAlgebra2 {
	...
	public Node operationValue(int opIndex, Node[] args) {
	...
	protected final Node spike(int a, Node rest) {
		for (int i = 0; i < algebraSize; ++i)
			subNodes[i] = next.emptySet;
	...
	public Node closure(Node a) {
		...
		while (completed < operations.length) {
			for (;;) {
				Node[] args = new Node[operations[index].operation.arity()];
