...
public class Engine {
	...
	public void mergeRecord(String parent, Record record, Table table,
			DataProvider provider) throws IOException {
	...
	public void mergeTable(String parent, Table source, Table table,
			DataProvider comparator) throws IOException {
	...
	public void processSources() throws ParseException, IOException {
		try {
			Date yesterday = new Date(new Date().getTime() - 24 * 60 * 60
					* 1000);
			...
			for (Record record : sources) {
				...
				Table table = provider.getPartialRecords(record
						.getValue("query"));
				mergeTable(record.getValue("uuid"), table,
						getTable(providerName), provider);
