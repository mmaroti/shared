...
/**
 * A database backed by an Microsoft Excel file. Each sheet contains a table.
 * The first row in the sheet defines the column (key) names. The subsequent
 * rows are the values. You can format and rearrange the columns, those
 * formating will not be lost when you save your data.
 */
public class ExcelDatabase {
		...
		for (Sheet sheet : workbook.getSheets()) {
			...
			if (cells.length > 0) {
				for (int j = 1; j < sheet.getRows(); ++j) {
					...
					for (int i = 0; i < keys.length; ++i)
						record.setValue(keys[i], sheet.getCell(i, j)
								.getContents());
	...
	public void save(String filename) throws Exception {
		...
		for (Table table : tables) {
			System.out.println("Saving " + table.getName() + " with "
					+ table.getSize() + " records");

			WritableSheet sheet;

			sheet = workbook.getSheet(table.getName());
			if (sheet == null)
				sheet = workbook.createSheet(table.getName(),
			...
			while (iterator.hasNext()) {
				...
				for (int i = 0; i < keys.length; ++i)
					sheet.addCell(new Label(indices[i], j, record
							.getValue(keys[i])));
