...
/**
 * A class to download data and manage from Google Scholar. This does
 * not do any fancy processing, just focuses on Google Scholar type
 * of data.
 */
public class GoogleScholar extends DataProvider
{
	...
	/**
	 * Downloads a webpage from Google Scholar, and handles all 
	 * necessary cookies. 
	 */
	public Document getDocument(String url) throws IOException
	{
		if( --maxConnections <= 0 )
			throw new TooManyConnectionsException("Too many Google Scholar HTML requests");
	...
	/*
	 * Returns a list of records from the Google Scholar URL. This method automatically
	 * follows the NEXT link, so your URL must start at the first record. Each record
	 * has the following fields:
	 * 
	 *	title:   publication title (stable)
	 *	url:     main URL of the publication (stable, not always present)
	 *	year:    publication year (optional)
	 *	doi:     DOI identifier (optional)
	 *	origin:  some info about the authors, journal and publisher (unreliable)
	 *	cites:   number of citations (optional, unstable)
	 *	citeid:  Google Scholar identifier for the list of citations (optional, unstable)
	 *	infoid:  Google Scholar identifier for the BiBTeX entry (optional, unstable)
	 *	britid:  British Library Direct identifier (optional)
	 */
	public Table getRecordsByUrl(String url) throws IOException
	{
		MemoryTable records = new MemoryTable("scholar", 
				new String[] {"title", "url", "year", "doi", "origin", "cites", "citeid", "infoid", "britid"});
			...
			for(Element element : elements)
			{
				...
				else
				{
					...
					if( ! title.startsWith("[CITATION]") )
						throw new IllegalArgumentException("Unexpected title format for scholar record");
				...
				for(Element link : links)
				{
					...
					else if( text.equals("BL Direct") )
					{
						...
						if( ! matcher.find() )
							throw new IllegalArgumentException("BL Direct url is not well formatted"); 
	...
	public Table getRecordsByAuthor(String author, String subject) throws IOException
	{
	...
	{
		String url = "http://scholar.google.com/scholar?cites=" + citeid + "&as_sdt=2005&sciodt=1,5&hl=en&num=100";
	...
	static protected MemoryTable bibtexTable = new MemoryTable("bibtex", 
			new String[] {"infoid", "bibid", "type", 
			"title", "author", "journal", "pages", "year", "volume", "number", "issn", 
			"book", "isbn", "publisher"});
	...
	public Record getBibTexRecord(String infoid) throws IOException
	{
		...
		Document doc = getDocument("http://scholar.google.com/scholar.bib?q=info:" + infoid + ":scholar.google.com/&output=citation&hl=en");
	...
	{
		if( a.getValue("infoid").length() >= 10 && a.getValue("infoid").equals(b.getValue("infoid")) )
			return DataProvider.IDENTICAL;

		if( a.getValue("citeid").length() >= 15 && a.getValue("citeid").equals(b.getValue("citeid")) )
			return DataProvider.IDENTICAL;

		if( a.getValue("url").length() >= 15 && a.getValue("url").equals(b.getValue("url")) )
			return DataProvider.IDENTICAL;
			
		if( a.getValue("title").length() >= 15 && a.getValue("title").equals(b.getValue("title")) 
				&& a.getValue("origin").length() >= 10 && a.getValue("origin").equals(b.getValue("origin")) )
			return DataProvider.IDENTICAL;
			
		if( a.getValue("doi").length() >= 10 && a.getValue("doi").equals(b.getValue("doi")) )
			return DataProvider.EQUIVALENT;
			
		if( a.getValue("britid").length() >= 10 && a.getValue("britid").equals(b.getValue("britid")) )
			return DataProvider.EQUIVALENT;

		if( a.getValue("title").length() >= 10 && getLevenshteinDistance(a.getValue("title").toUpperCase(), b.getValue("title").toUpperCase()) <= 5 )
	...
	public Table getPartialRecords(String query) throws IOException
	{
		...
		if( options.containsKey("author") )
			table = getRecordsByAuthor(options.get("author"), options.get("subject"));
		else
	...
	protected static MemoryTable fullTable = new MemoryTable("scholar-full", 
			new String[] {"title", "url", "year", "doi", "origin", "cites", "citeid", "infoid", "britid",
			"bibid", "type", "author", "journal", "pages", "volume", "number", "issn", 
			"book", "isbn", "publisher"});
	...
	public Record getFullRecord(Record partialRecord) throws IOException
	{
		...
		{
			Record bibtexRecord = getBibTexRecord(partialRecord.getValue("infoid"));
