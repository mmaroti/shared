...
/**
 * A structure whose operation tables are explicitly stored in tables.
 * Sometimes, it is advisable to copy small derived structures to
 * StructureBuffers to speed up operations.
 *
 * @author mmaroti@math.u-szeged.hu
 */
public class StructureBuffer extends Structure
{
	...
	/**
	 * Constructs a structure with no operations and relations.
	 */
	public StructureBuffer(Signature signature, int size)
	{
		this.size = size;
		universe = new FiniteSet(size);
		
		if( size <= 0 )
			throw new IllegalArgumentException("the size of a structure must be positive");

		Symbol[] ops = signature.getRelations();
		relations = new Rel[ops.length];
		for(int i = 0; i < relations.length; ++i)
			relations[i] = new Rel(ops[i]);
	}

	/**
	 * Creates a copy of a structure.
	 */
	public StructureBuffer(Structure structure)
	{
		size = structure.getSize();
		universe = new FiniteSet(size);
				
		Relation[] ops = structure.getRelations();
		relations = new Rel[ops.length];
	...
	/**
	 * The list of fundamental operations of the structure.
	 * @see #getRelations
	 ...
	 */
	public final StructureBuffer.Rel getRelationTable(int index)
	{
	...
	public class Rel extends Relation
	{
		...
		{
			if( symbol.arity != 1 )
				throw new UnsupportedOperationException("this relation is not unary");
		...
		public Rel(Relation relation)
		{
			...
			if( size != relation.getUniverse().getSize() )
		...
		/**
		 * Returns an array holding the underlying relation table. 
		...
		/**
		 * Evaluates the mapping at the given arguments.
		 * This function is equivalend to 
