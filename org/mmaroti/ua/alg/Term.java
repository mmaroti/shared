...
{
	/**
	 * Constructs a term whose topmost operation is <code>symbol</code>
	 * and subterms are <code>subterms</code>.
	 */
	 ...
	 * variable then this method returns <code>null</code>.
	 */
	...
	{
		// TODO: how can the symbol be null?
		if( subterms[index].symbol == null ||
			! symbol.isBraced(index, subterms[index].symbol) )
	...
	public String toString()
	{
		...
		if( symbol.hasProperty(Symbol.INFIX) )
		{
			if( subterms.length == 0 )
				...
				return subtermToString(0) + symbol.getName() + subtermToString(1);
		}
	...
	protected static Term parseSubterm(Symbol symbol, int index, TermAlgebra algebra, String substring)
	{
		Term term = parse(algebra, substring);
		
		if( term != null && symbol.isBraced(index, term.symbol) && ! bracedLastParse )
			term = null;
	...
	public static Term parse(TermAlgebra algebra, String string)
	{
		...
		for(int i = 0; i < ops.length; ++i)
		{
			...
			if( symbol.hasProperty(Symbol.INFIX) )
			{
				...
				{
					Term subterm = parseSubterm(symbol, 0, algebra, string.substring(name.length()));
				...
				else if( symbol.getArity() == 2 )
				{
					...
					for(;;)
					{
						...
						Term subterm1 = parseSubterm(symbol, 0, algebra, string.substring(0, pos));
						Term subterm2 = parseSubterm(symbol, 1, algebra, string.substring(pos + name.length()));
						
						if( subterm1 != null && subterm2 != null )
						{
							bracedLastParse = false;
							return new Term(symbol, new Term[] { subterm1, subterm2 });
						}
					}
				}
			}
			...
			{
				String[] substrings = parser.parseList(string.substring(name.length() + 1, string.length() - 1), ",");
	...
	public void addMyVariablesTo(SubUniverse set)
	{
		...
		{
			for(int i = 0; i < subterms.length; ++i)
				subterms[i].addMyVariablesTo(set);
		}
		else
			set.add(symbol);
	}
	 ...
	 * 
	 * @param subterm another term of this algebra
	 * @return <code>true</code> if <code>subterm</code> is
