...
/**
 * A class representing operation and relation symbols. The symbol name,
 * arity and special properties are stored in this object.
 *  
...
public class Symbol
{
     ...
     */
    public Symbol(int index, String name, int arity, int priority, int properties)
    {
		if( arity < 0 )
    ...
    /**
     * Returns the priority of the operation. Higher priority operations
     * are performed first, then lower priority ones. For example, the priority
     * of multiplication is a higher number than that of addition.
     */
	 ...
	 * 
	 * @param index The coordinate of this symbol into which the argument is
	 * 	plugged in.
	 * @param argument The topmost operation symbol of the argument. If 
	...
	public boolean isBraced(int index, Symbol argument)
	{
		...
		{
			if( argument == null || argument.priority > priority || ! argument.hasProperty(INFIX) )
				return false;
			
			if( arity == 2 && index == 0 && hasProperty(LEFT_ASSOCIATIVE) && argument == this )
				return false;
				
			if( arity == 2 && index == 1 && hasProperty(RIGHT_ASSOCIATIVE) && argument == this )
				return false;
	...
	/**
	 * The symbol is a infix operation. If an operation is not infix, then
	 * it will be expressed as a function.
	 */
    ...
    /**
     * The index of a symbol is used to quickly find corresponding
     * operations of different signatures. Also, variables in terms
     * are differentiated by this index field
     */
    protected int index;
	...
	public static Universe VARIABLES = new Universe()
	{
		public int getSize()
		{
			throw new UnsupportedOperationException("variables are not enumerable");
		}
