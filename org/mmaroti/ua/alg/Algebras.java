...
/**
 * This class contains various static helper methods and classes
 * for algebras and operations.
 * 
...
public class Algebras
{
	 ...
	 * @param arity The dimension of the table.
	 * @param padding If the returned entry is shorter than padding,
	 * it will be padded by spaces on the left.
	 * @param writer The writer used to print the table.
	 * @param formatter The formatter used to get the elements of the table,
	 */
	...
	/**
	 * Prints an algebra to the specified writer. This method prints
	 * the operation and relation tables using the indices. It completely
	 * disregards the labeling information.
	 * 
	 * @throws UnsupportedOperationException if the algebra cannot be enumerated 
	 */
	public static void printTo(Algebra algebra, XmlWriter writer)
	{
		...
		printUniverseTo(algebra.getUniverse(), writer);
		...
		for(int i = 0; i < ops.length; ++i)
		{
			...
			printTable(size, op.getSymbol().getArity(), 
				size <= 99 ? 2 : size <= 999 ? 3 : 1, writer,
				new EntryFormatter()
				{
					public String getLabel(int[] args)
					{
	...
	public static void printUniverseTo(Universe universe, XmlWriter writer)
	{
		...
		for(int i = 0; i < size; ++i)
			writer.printLine("" + i + " : " + universe.toString(universe.getElement(i)));
