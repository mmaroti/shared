...
public final class IntegerMax extends BinaryOp<Integer, int[]> {
	private IntegerMax() {
	...
	public void apply(int[] src1, int src1Pos, int[] src2, int src2Pos,
		...
		for (int i = 0; i < count; i++)
			dst[dstPos + i] = Math.max(src1[src1Pos + i], src2[src2Pos + i]);
	}
	...
	public void apply(int[] src, int srcPos, int[] dst, int dstPos, int count) {
		...
		for (int i = 0; i < count; i++)
			dst[dstPos + i] = Math.max(dst[dstPos + i], src[srcPos + i]);
	}
	...
	public void apply(int[] src, int srcPos, Integer elem, int[] dst,
		...
		int b = elem;
		for (int i = 0; i < count; i++)
			dst[dstPos + i] = Math.max(src[srcPos + i], b);
	}
	...
	public void apply(Integer elem, int[] dst, int dstPos, int count) {
		...
		int b = elem;
		for (int i = 0; i < count; i++)
			dst[dstPos + i] = Math.max(dst[dstPos + i], b);
	}
